...
# Modified convertors for dealing with the fused models
...
def hf_to_tune_fused(
    ...
    for key, value in state_dict.items():
        if "rotary_emb.inv_freq" not in key:  # Skip loading the position embeddings
            new_key = get_mapped_key(key, _FROM_HF_FUSED)
            if "qkv_proj" in key:
                q, k, v = torch.split(value, [dim1, dim2, dim2], dim=0)
                q = _permute(q, num_heads)
                k = _permute(k, num_kv_heads)
                value = torch.cat([q, k, v], dim=0)
            converted_state_dict[new_key] = value
    return converted_state_dict


def tune_to_hf_fused(
    state_dict: Dict[str, torch.Tensor],
    num_heads: int = 32,
    num_kv_heads: int = 32,
    dim: int = 4096,
